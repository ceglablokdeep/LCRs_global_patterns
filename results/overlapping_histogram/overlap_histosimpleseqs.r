...
if (length(args)<1) {
  stop("Please provide arguments like Rscript overlap_histosimpleseqs.r cladename [the values in file should look like this: XP_006877394.1_Chrysochloris_asiatica 519 395 423 9 {P}]", call.=FALSE)
}
library(stringr)
cname=args[1]
clade=str_to_title(cname)
fname=paste(cname,"_all_flps_combined",sep="")
r=read.table(fname,header=F)
...
jpeg(paste('overlap_of_repeatgrads_in_',cname,'_in_all_genes.jpeg',sep=''),width=28,height=18,units="in",res=300)
par(mar=c(6,7,5,2))
mtitle=paste("Distribution of low-complexity regions in",clade,"on normalized gene position",sep=" ")
...
pr70t=paste('Low-complexity regions of > 70% purity ( n =',nrp70,')')
