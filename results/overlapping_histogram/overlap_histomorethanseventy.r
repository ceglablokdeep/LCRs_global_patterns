...
if (length(args)<2) {
  stop("Please provide arguments like Rscript overlap_histomorethanseventy.r afrotheria_flps_morethanseventy_repeats cladename [the values in file should look like this: XP_006877394.1_Chrysochloris_asiatica 519 395 423 9 {P}]", call.=FALSE)
}
library(stringr)
...
jpeg(paste('overlap_of_cbrmorethanseventy_and_pssites_in_',cname,'_in_all_genes.jpeg',sep=''),width=28,height=18,units="in",res=300)
par(mar=c(6,7,5,2))
mtitle=paste("Distribution of compositionally-biased of > 70 % purity regions and positively selected sites in",clade,"on normalized gene position",sep=" ")
...
pr1=paste('Compositionally-biased regions of > 70% purity ( n =',nrp,')')
