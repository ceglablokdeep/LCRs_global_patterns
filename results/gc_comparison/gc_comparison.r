data=read.table("gcpercompiled_arranged",header=F)
colnames(data)=c("clades","gc","status")
data_new <- data
...
image_name=paste('gc_cladewise_LPSs_nonLPSs_comparison.jpeg',sep="")
jpeg(image_name,width=28,height=18,units="in",res=300)
par(mar=c(5,4,4,1))
myplot <- boxplot(gc ~ status*clades, data=data_new,ylim=c(-0.75,max(data_new$gc)+2), 
        boxwex=0.4,outline=F,
        ann=F,
        col=c("seagreen4" , "coral3") , 
        border=c("seagreen3","coral2"),cex.axis=1.25,
        xaxt="n")
maintext=paste("%GC variation in proteins with LCRs and proteins without LCRs",sep="")
...
legend(x=21,y=18, legend = c("Proteins with LCRs", "Proteins without LCRs"), 
       col=c("seagreen4" , "coral3"),
...
b1=b1[!is.na(as.numeric(b1$gc)), ]
a1=a1[!is.na(as.numeric(a1$gc)), ]
yax=max(quantile(a1$gc,probs=0.75,na.rm=T),quantile(b1$gc,probs=0.75,na.rm=T))+0.2
...
yminax=min(quantile(a1$gc,probs=0.25,na.rm=T),quantile(b1$gc,probs=0.25,na.rm=T))-0.2
text(xpos,yminax-3,bquote(bar("x") == .(a1mean)),cex=1.1)
text(xpos+1,yminax-3,bquote(bar("x") == .(b1mean)),cex=1.1)
text(xpos,yminax-6.25,bquote("n" == .(dim(a1)[1])),cex=1.1)
text(xpos+1,yminax-6.25,bquote("n" == .(dim(b1)[1])),cex=1.1)
xpos=xpos+2
}
dev.off()
